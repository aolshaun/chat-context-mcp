 ...
 */
export function formatAsTable(sessions: SessionMetadata[]): string {
 ...
 */
export function formatAsCompact(sessions: SessionMetadata[]): string {
  if (sessions.length === 0) {
    return chalk.yellow('No sessions found.');
  }
 ...
 */
export function formatSession(session: any, format: 'json' | 'markdown' | 'table' | 'compact'): string {
  switch (format) {
    case 'json':
 ...
 */
function formatSessionMetadataTable(metadata: SessionMetadata): string {
  const colors = useColors();
  const table = new Table({
    colWidths: [20, 50]
  });
 ...
 */
export function formatStatsTable(stats: any): string {
  const colors = useColors();
  const table = new Table({
    colWidths: [35, 15]
  });
