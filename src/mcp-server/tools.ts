...
import { CursorContext, formatSessionMarkdown, formatSessionJSON } from '../core/index.js';

/**
 * List sessions
 */
export async function handleListSessions(api: CursorContext, args: any) {
 ...
 * Search sessions
 */
export async function handleSearchSessions(api: CursorContext, args: any) {
  if (!args.query) {
 ...
 */
export async function handleGetSession(api: CursorContext, args: any) {
 ...
 */
export async function handleNicknameCurrentSession(_api: CursorContext, args: any) {
  if (!args.nickname) {
 ...
 */
export async function handleSetNickname(api: CursorContext, args: any) {
  if (!args.sessionId || !args.nickname) {
    throw new Error('sessionId and nickname are required');
  }

  await api.setNickname(args.sessionId, args.nickname);

  return {
    content: [
      {
        type: 'text',
        text: `✓ Nickname "${args.nickname}" set for session ${args.sessionId.substring(0, 8)}...`,
      },
    ],
  };
}
 ...
 */
export async function handleAddTag(api: CursorContext, args: any) {
 ...
 */
export async function handleRemoveTag(api: CursorContext, args: any) {
  if (!args.sessionId || !args.tags) {
    throw new Error('sessionId and tags are required');
  }
 ...
 * List all tags
 */
export async function handleListTags(api: CursorContext) {
 ...
 * List all projects
 */
export async function handleListProjects(api: CursorContext) {
