...
The nickname will be applied when this session is synced to the database.`,
        ...
        {
          name: 'set_nickname',
          description: `Set a nickname for a PAST session (requires session ID).

Use when user wants to name a PAST session:
- "Nickname session abc123 'auth-implementation'"
- "Rename that old chat to 'database-design'"

Note: Requires the session ID (UUID). Use list_sessions or search_sessions to find session IDs.`,
          inputSchema: {
            type: 'object',
            properties: {
              sessionId: {
